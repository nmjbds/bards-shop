...
// below (signup, signin, OAuth, Telegram, profile, etc.) calls issueSession/
// isSuspended/safe exactly as before; only where the code physically lives
// changed, not what it does.
const {
  issueSession, isSuspended, SUSPENDED_MSG, safe,
...
// /refresh, /logout, /me — handlers now live in services/session.js (see
// import above and that file's header comment). Registered here exactly as
// before so the customer server's routes are unchanged; routes/authSession.js
// registers the same three handlers on the seller./admin. servers.
...
// safe() also moved to services/session.js — imported above with the rest.

// Additive-only exports (2026-07-28, multi-domain split prep) — everything
// above this line is completely unchanged in behavior. routes/authSession.js
// (mounted at /api/auth on the seller./admin. servers, which have no
// signin/signup pages of their own) reuses refreshHandler/logoutHandler/
// meHandler directly instead of a second copy, so there's exactly one place
// that implements session refresh/revoke/lookup — not three. The old
// `{ router, passport }` shape is untouched; server.js's existing
// `const { router: authRouter, passport } = require('./routes/auth')`
// destructure ignores the extra fields, so this is safe for the still-live
// combined service too.
module.exports = { router, passport, refreshHandler, logoutHandler, meHandler };